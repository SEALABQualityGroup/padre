...
public class Do_OperationsOnDB extends ViewPart {
				...
				try {
					...
					for (Integer Onlineid : do_id_list_Online) {
						...
						for (Integer local_id : do_id_list) {
							...
							Do_operation redoOnline = Db.get_Do_description_byId_Online(Onlineid);
							Do_operation redo = Db.get_Do_description_byId(local_id);
							...
							if (redo.getName().equals(redoOnline.getName())) {
								...
								if (compare(LocalOperation, OnlineOperation)) {
	...
	public void chooseDo() {
		...
		try {
			...
			Do redo = (Do) itemSelected.getData();
